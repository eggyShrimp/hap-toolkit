...
校验器 SHALL 读取 manifest 中 `router.widgets` 的 `backgroundType` 声明：未声明时给出 INFO 提示并默认按 `custom` 处理；取值不是 `solid` / `custom` 时给出 ERROR。
...
对声明 `backgroundType: "solid"` 的轻卡，校验器 SHALL 校验根节点背景：必须设置在根节点、无背景图/渐变、浅色为 `#FFFFFF`/`#F5F5F5`、深色经 `@media (prefers-color-scheme: dark)` 设为 `#1A1A1B`、alpha 为 1。
...
- **WHEN** solid 卡片根节点存在 `background-image`（含渐变）
- **THEN** 输出 ERROR 提示不允许使用 background-image/渐变
...
- **WHEN** 深色媒体规则中的根节点背景不是 `#1A1A1B`
- **THEN** 输出 ERROR 提示仅支持 `#1A1A1B`

### Requirement: 背景透明度校验

对 `solid`、`custom` 及未声明 `backgroundType` 的轻卡，校验器 SHALL 拒绝根节点背景透明度（alpha < 1），包括 `rgba()`、8/4 位 hex 与 `transparent` 等形式。
