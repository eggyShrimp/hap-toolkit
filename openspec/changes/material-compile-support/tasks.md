...
- [ ] 3.1 新增 `packages/hap-packager/src/plugins/material-validate-plugin.js`：`PROCESS_ASSETS_STAGE_ANALYSE` 阶段读取 manifest 与产物，输出带卡片定位的 warnings
- [ ] 3.2 在 `packages/hap-packager/src/plugins/index.js` 导出插件，并在 `webpack.post.js` 的 `CardPlugin` 之后注册
- [ ] 3.3 补充插件层测试：`disabledRules` / `silentInfo` / 非 lite 卡片跳过

## 4. 构建与冒烟验证

- [ ] 4.1 `npm run build` 后运行定向单测（compiler + packager 新增用例全部通过）
